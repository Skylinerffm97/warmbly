...
`/data/blobs` is a hand-off, not a destination: it is the one path the container and the host both see. The third line matters. `backup` leaves its own output out of the archive, but a bundle left there is backed up by the next run, so move it out and delete it.
