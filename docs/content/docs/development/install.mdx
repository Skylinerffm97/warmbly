...
A real path is the default, because it is the choice that makes backup and migration obvious: moving the instance to another host is `rsync` of that path plus the `.env`. Compose reads a volume source starting with `/` as a bind mount and anything else as a named volume, so this is one variable per store and no second compose file.
